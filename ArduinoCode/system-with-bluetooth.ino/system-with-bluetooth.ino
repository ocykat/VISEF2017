...
int relay_pump_LockTime = 300;
int relay_pump_OperateTime = 3;
...
int ledMode = -1; // -1 = turn ON/OFF according to day time
                  // 0 = manual control: led OFF
                  // 1 = manual control: led ON
int pirMode = 1; // 0 = pir OFF
                 // 1 = pir ON
...
int pumpprevMode;
int pumpMode = -1; // -1 = turn ON/OFF according to set time
                   // 0 = manual control: pump OFF
                   // 1 = manual control: pump ON
...
void lcd_show_ledActiveTime(int hour, int minute) {
    if ((!lcd_Locked) && (Daytime) && (lcd_tracker >= 9)) {
        lcd_tracker++;
        lcd_clearline(1);
        lcd.setCursor(0, 1);
        lcd.print("DenSang");
        lcd.setCursor(9, 1);
        lcd.print(lcd_twochar(hour - StartofDay) + ":" + lcd_twochar(minute));
        if (lcd_tracker == 12) {
            lcd_tracker = 0;
        }
    }
...
void sm_controls() {
    if (!sm_Locked) {
        ...
        Serial.print("Pir Mode: "); Serial.println(pirMode);
...
void Serial2_ReadfromApp() {
    ...
    if (Serial2_cmd.startsWith("Wifi")) {

    }
    else if (Serial2_cmd.startsWith("LED")) {
        if (Serial2_cmd.endsWith("0")) {
            ledMode = 0;
        }
        else if (Serial2_cmd.endsWith("1")) {
            ledMode = 1;
        }
        else {
            ledMode = -1;
    ...
    else if (Serial2_cmd.startsWith("PIR")) {
        if (Serial2_cmd.endsWith("0")) {
            pirMode = 0;
        }
        else {
    ...
    else if (Serial2_cmd.startsWith("PUMP")) {
        if (Serial2_cmd.endsWith("0")) {
            pumpMode = 0;
        }
        else if (Serial2_cmd.endsWith("1")) {
    ...
    else if (Serial2_cmd.startsWith("RTC")) {
...
void loop() {
    ...
    if (!Initialized) {
        ...
        // Receiving cmd from app       
        Serial2_ReadfromApp();

        // Time management
        DayTimeCheck(now.hour());
